...
export default function SettingsModal({
  ...
  const { busy: transferring, tasks } = useTaskStore();
  ...
  const onRefreshSystemCheck = useSystemStore(s => s.refresh);

  const gameRunning = useGameStore(s => s.running);
  const refreshGameState = useGameStore(s => s.refreshGameState);

  const onSync = async () => {
    await Promise.all([reloadSettings(), onRefreshSystemCheck(), refreshGameState()]);
  };
  ...
  const load = async () => {
    ...
    try {
      ...
      if (fresh.status === 'error') throw new Error(fresh.error);
      if (!edited.current) {
        setForm(fresh.data);
        setBaseline(fresh.data);
      }
    } catch (e) {
      setLoadError(e);
    }
  ...
  useEffect(() => {
    ...
    commands.getSettings()
      .then((fresh) => {
        if (active && !edited.current && fresh.status === 'ok') {
          setForm(fresh.data);
          setBaseline(fresh.data);
        } else if (active && fresh.status === 'error') {
          setLoadError(new Error(fresh.error));
        }
      })
      .catch((e) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await onSave(next);
      const savedRes = await commands.getSettings();
      if (savedRes.status === 'error') throw new Error(savedRes.error);
